...
- 自动打开好友列表
- 自动拜访好友
- 识别好友家的一键摘取、摘取手形、一键务农、单个作物“可摘”等动作按钮
...
- 当前可见好友都访问过时，默认重新访问最上方好友
   ...
   默认识别好友列表里的收获标记和务农标记：
   - "friend_action_harvest" 手形/面罩，表示可收获或可摘
   - "friend_action_hoe" 小锄头，表示可务农

   behavior.enable_crop_pickable = true 时，进入好友家后会在一键动作之后继续识别并点击农田里的“可摘”浮标。

   behavior.farm_button_click_probability 控制“一键务农”的点击概率，默认 0.3。识别到“一键摘取”或摘取手形时仍然必点，不受这个概率影响。
## 使用方法
...
| `friend_action_harvest.png` | 好友列表里的收获/可摘帮忙标记 |
...
| `crop_pickable.png` | 好友家农田里的“可摘”浮标 |
...
如果日志里出现 “macOS 未授权当前终端发送鼠标事件”，说明截图可以工作但点击事件被系统拦截，需要重新检查辅助功能授权。
